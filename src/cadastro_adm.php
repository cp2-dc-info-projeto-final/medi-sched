...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Verifica se o email já está cadastrado na tabela de Administrador
    ...
    if ($stmt->num_rows > 0) {
        ...
        $stmt->close();
        // Se não estiver cadastrado, insere o novo administrador
        $senhaHash = password_hash($senha, PASSWORD_DEFAULT);
        $stmt = $mysqli->prepare("INSERT INTO Administrador (nome_administrador, sobrenome_administrador, email, senha, cpf, data_nascimento, genero) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssss", $primeiro_nome, $sobrenome, $email, $senhaHash, $cpf, $data_nascimento, $genero);

        if ($stmt->execute()) {
            header("Location: index_adm.php"); // Redireciona para a página principal do administrador
            exit;
        } else {
